...
| `dangerWeight` | float | 1.0 | How heavily an active/incubating case weighs toward the Mode 2 director's colony disease burden. Higher = the director backs off new seeding harder while this disease is present (dangerous diseases buy more breathing room). |
...
- **Sick signal behavior:** `Contagion_AnimalSick` is static (no severity progression) and self-clears if untreated by rolling once per day: 20%, then +10 percentage points per day, with a forced clear on day 5. It blocks the animal from the auto-slaughter queue while present. A permanent vanilla `HediffComp_TendDuration` marks it non-tendable after the first treatment as a fail-safe; normal diagnosis removes it immediately.
- **Diagnosis:** Vanilla `DoctorTendToAnimals` and `JobDriver_TendPatient` own automatic diagnosis. The signal's `CompTended` hook performs one unified diagnostic roll (`ContagionDiagnosticSkillUtility`) and always resolves the signal:
  ...
  The sick signal never counts toward vanilla's required medicine total and never consumes carried medicine. A signal-only tend job is therefore created without a medicine target or reservation. If diagnosis reveals a real disease, that disease is handled by a fresh normal vanilla tend job with the animal's configured medical-care rules.
...
| Fomites limited to visible, cleanable filth | Started vomit-only (visible and cleanable; no invisible contamination). Now also covers contaminated animal filth (fecal-oral living vector) and corpse fluids/fleas — all visible, locatable filth or corpses the player can clean or remove, preserving the "no invisible contamination" principle. |
| Fecal-oral split: invisible nodes are **outdoor-only**; indoors is cleanable filth | The eating route's abstract `_hotspots` (the "soil-mixing" model) are the one tolerated *invisible* contamination — acceptable only outdoors, where there's no bounded space to clean and they self-decay. **Indoors (enclosed psychologically-indoor room) no node is ever minted**; all indoor fecal-oral risk flows through cleanable `Filth_AnimalFilth` via the living route, so a clean enclosed room prevents infection. This keeps "no invisible contamination" intact wherever the player can actually act. **Do not re-enable indoor node shedding** — `RunFecalOralEatingSheddingPass` gates `_hotspots.AddOrRefresh` on `!IsIndoorBarnCell`, and `NotifyAnimalIngested` early-returns on `context.IsIndoor` as the guard for nodes whose cell is later enclosed. "Indoor" = enclosed room (`!PsychologicallyOutdoors`), **not** bare roof: a single shared `ContagionTransmissionUtility.IsIndoorBarnCell` is the sole authority for shedding suppression, filth registration, and ingestion classification — keep them on that one predicate. Roofed-but-open sheds are outdoor (the room-ambient living route can't bound an open area). |
...
| Automatic diagnosis is a `CompTended` hook inside vanilla tending | Reuses vanilla doctor scanning, reservations, timing, interruption, and XP behavior while keeping the sick signal medicine-free. The separate proactive examination remains a player-ordered short interaction only |
